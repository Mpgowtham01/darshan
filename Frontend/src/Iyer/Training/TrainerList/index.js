...
const trainerlist = [
  {
    ...
    trainerDescription:
      "The Hanuman Chalisa Trainer is a dedicated learning tool designed to help individuals memorize, recite, and understand the Hanuman Chalisa, a devotional hymn dedicated to Lord Hanuman",
  ...
  },
  {
    ...
    trainerDescription:
      "The Ramayana Trainer is a comprehensive educational tool designed to help individuals learn, understand, and immerse themselves in the epic tale of the Ramayana. This trainer caters to different learning styles and preferences, ensuring a holistic understanding of the text, its characters, and its cultural significance. ",
    startDate: "1st July 2024",
    endDate: "30th July 2024",
    startTime: "5.00 PM",
    endTime: "6.00 PM",
    startDay: "Monday",
    endDay: "Friday",
...
export default function TrainerList() {
  ...
  const [filteredClassList, setFilteredClassList] = useState(trainerlist);
  ...
  const getTrainingDetails = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_DEV_BASE_URL}/trainerclasslist/getone/${id}`
      )
      .then((res) => {
  ...
  const applyFilters = () => {
    let filteredList = trainerlist;

    if (online && !offline) {
      filteredList = filteredList.filter((item) => item.mode === "Online");
    } else if (offline && !online) {
      filteredList = filteredList.filter((item) => item.mode === "Offline");
    }
    ...
    if (dist) {
      filteredList = filteredList.filter((item) => item.district === dist);
    }
  ...
  useEffect(() => {
    applyFilters();
  }, [online, offline, all, languageValue, country, state, dist, city]);
            ...
            <div>
              ...
              <div className="d-flex justify-content-center">
                <Image
                  alt="Pooja images*****"
                  width={350}
                  src={`${process.env.REACT_APP_DEV_BASE_URL}${classDetail.images}`}
                  height={330}
                />
                {console.log("img:", classDetail.image)}
              </div>
              <p className="mt-4 mx-5" style={{ textAlign: "justify" }}>
                {classDetail.description}
              </p>
            </div>
            <div className="row mt-5 mb-3">
              <h5>Trainer's List</h5>
              {filteredClassList.map((list, index) => (
