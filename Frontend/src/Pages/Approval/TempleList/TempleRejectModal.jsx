...
const TempleRejectModal = ({ show, handleClose, handleReject, id }) => {
    const [rejectReason, setRejectReason] = useState("");

    const handleSubmit = async (e) => {
        try {
            e.preventDefault();
            const result = await axios.put(`${process.env.REACT_APP_DEV_BASE_URL}/temple/updateStatus/${id}`, { templeApproveStatus: "2", reason: JSON.stringify(rejectReason || "") });
            if (result.data) {
                handleReject();
                handleClose();
            }
...
export default TempleRejectModal;