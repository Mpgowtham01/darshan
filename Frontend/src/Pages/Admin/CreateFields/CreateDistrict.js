...
const DistrictForm = () => {
  ...
  const displayToaster = () => {
    toast.current.show({
      severity: "success",
      summary: "Done",
      detail: `District ${locationState ? "is Updated" : "Created"}`,
      life: 3000,
    });
  ...
  const onSubmit = async data => {
    ...
    try {
      if (locationState) {
        //update
        await axios.put(
          `${process.env.REACT_APP_DEV_BASE_URL}/district/update/${locationState.id}`,
          data
        );
        displayToaster();
      } else {
        //create
        await axios.post(
          `${process.env.REACT_APP_DEV_BASE_URL}/district/create`,
          data
        );
        displayToaster();
      }
  ...
  return (
    <div className="createListForm">
      ...
      <Container>
        <h1 className="createListForm__title">Create District</h1>
        <section>
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="d-flex justify-content-end py-3">
              ...
              <button className="customBtn small bg" type="reset" onClick={() => { navigate("/admin/district"); }}>
                Cancel
              </button>
            </div>
          </Form>
