...
export default function CreateEventType() {
  ...
  const onSubmit = async (data, e) => {
    try {
      if (locationState) {
        //update
        await axios.put(
          `${process.env.REACT_APP_DEV_BASE_URL}/eventType/updateSingleEventType/${locationState.id}`,
          data
        );
        displayToaster();
      } else {
        //create
        await axios.post(
          `${process.env.REACT_APP_DEV_BASE_URL}/eventType/createNewEventType`,
          data
        );
        displayToaster();
      }
  ...
  return (
    <div className="createListForm">
      ...
      <Container>
        <h5 className="createListForm__title">Create EventType</h5>

        <section>
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Form.Group className="mb-3">
              <Form.Label className="font-title">Event Name</Form.Label>
