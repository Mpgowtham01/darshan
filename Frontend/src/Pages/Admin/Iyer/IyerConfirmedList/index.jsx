...
import IyerConfirmedModal from "./IyerConfirmedModal";
...
const Action = ({ record }) => {
    ...
    return <>
        <IyerConfirmedModal show={show} id={record.iyer_booking_id} handleClose={() => setShow(false)} />
        <button className="btn btn-primary" style={btnStyles} onClick={() => setShow(true)}>View</button>
    </>;
};

export default function IyerConfirmedList() {
    ...
    const getIyerList = () => {
        axios
            .get(`${process.env.REACT_APP_DEV_BASE_URL}/iyerbooking/getIyerbookingDataFromAdmin/1/1`)
            .then(res => {
