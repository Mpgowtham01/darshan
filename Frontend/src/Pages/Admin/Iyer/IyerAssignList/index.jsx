...
import IyerAssignModal from "./IyerAssignModal";
...
const Action = ({ record }) => {
    ...
    return <>
        <IyerAssignModal show={show} id={record.iyer_booking_id} handleClose={() => setShow(false)} />
        <button className="btn btn-primary" style={btnStyles} onClick={() => setShow(true)}>View</button>
    </>;
};

export default function IyerAssignList() {
    ...
    const getIyerList = () => {
        axios
            .get(`${process.env.REACT_APP_DEV_BASE_URL}/iyerbooking/getIyerbookingDataFromAdmin/1/0`)
            .then(res => {
