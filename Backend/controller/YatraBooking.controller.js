...
yatraBooking.bookTrip = async (req) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const mainImagesJSON = JSON.stringify(mainImage);
      ...
      const sql = `INSERT INTO yatra_booking (title, from_location, to_location, designation, days, dayslist, price, offerPrice, overview, what_is_included, meeting_point, end_point, mainImage, images) 
                          VALUES ('${title}', '${from_location}', '${to_location}', '${designationJson}', '${days}', '${dayslistJson}', '${price}', '${offerPrice}', '${overview}', '${whatIsIncluded}', '${meetingPoint}', '${endPoint}','${mainImagesJSON}' '${imagesJson}')`;
